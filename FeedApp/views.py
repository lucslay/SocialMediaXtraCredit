...
def comments(request, post_id):
    if request.method == 'POST' and request.POST.get("btn1"):
        ...
        print(comment)
        Comment.objetcs.create(post_id=post_id,username=request,text=comment,
...
def friends(request):
    ...
    all_profiles = Profile.objects.exlude(user=request.user).exclude(id__in=user_friends_profiles).exclude(id__in=request_sent_profiles)
    ...
    if request.method == 'POST' and request.POST.get("sent_requests"):
    ...
    if request.method == 'POST' and request.POST.get("receive_requests"):
        ...
        for sender in senders:
            ...
            # to add the friends list of the user
